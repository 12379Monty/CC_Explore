...
# nohup Rscript -e "bookdown::render_book('index.Rmd')" > index.log  &
...
resoluti# Compare Predictive Models {#comp-models}
...
### BREAK CACHE
 ...
 function(PRED) {
  ...
  cnt=Set.Train.TruthTable.frm,
  prop=Set.Train.TruthTable.frm/length(Train.Label.vec))),
...
### BREAK CACHE
 ...
 knitr::kable(data.frame(cbind(
   cnt=Set.Train.osPredMode.TruthTable.frm,
   prop=Set.Train.osPredMode.TruthTable.frm/length(Train.Label.vec))),
...
### BREAK CACHE
 ...
 knitr::kable(data.frame(cbind(
    cnt=Set.Test.TruthTable.frm,
    prop=Set.Test.TruthTable.frm/length(Test.Label.vec))),
...
Train data accuracy is **always near perfect** for models that
have a large paremeter space such a random forest models.
This assessment is only included here for reference purposes -
we cannot judge the relative performance of models having
vastly different parameter spaces based on training data results.
...
Replot with just 5 classifiers to improve clarity with better resolution.

```{r comp-models-CompOOFSet-TrainProbClass1-5models, cache=FALSE, cache.vars='', fig.height=12, fig.width=12, eval=T, fig.cap='Predicted Probabilities'}

### BREAK CACHE

  par(mfrow=c(5,1), mar=c(0,5,2,1), oma=c(5,0,1,0))

  for(MOD in unique(Col.Models.vec)[1:5]) {
   Mod.cols <- which(Col.Models.vec==MOD)
